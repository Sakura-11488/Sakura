...
#   2. Copy the service into /opt/sakura/hentai-scraper
#   3. Install npm dependencies
#   4. Start (or reload) the PM2 process
#   5. Patch nginx with the /hentai/v1/ proxy snippet if not present
#   6. Reload nginx
...
if [[ -f "$NGINX_SITE" ]] && ! grep -q "$NGINX_SNIPPET_MARKER" "$NGINX_SITE"; then
    log "patching nginx site ($NGINX_SITE) with /hentai/v1/ proxy snippet"
    tmp="$(mktemp)"
...
log "WARN: health check did not return 200. Check 'pm2 logs sakura-hentai-scraper' for details."
exit 1
