...
function SakuraAIModal({ onClose }: { onClose: () => void }) {
    ...
    // Lock the page behind the modal so wheel/touch scrolling cannot bleed
    // through to the main app while the AI panel is open.
    useEffect(() => {
    ...
    const allowed = useMemo(() => {
        ...
        // The Android build gates AI access behind 100k $SAKURA because that
        // app pays for the model API on the user's behalf. The desktop build
        // proxies through the user's own local config (electron/sakura-ai.config.json),
        // so there is no shared backend to abuse — bypass the holders gate.
