...
export function detectDeviceLocale(): Locale {
  try {
    const resolved = Intl.DateTimeFormat().resolvedOptions().locale || DEFAULT_LOCALE;
    const normalized = resolved.toLowerCase();
    const exact = SUPPORTED_LOCALES.find((item) => item.code.toLowerCase() === normalized);
    if (exact) return exact.code;
    const base = SUPPORTED_LOCALES.find(
      (item) => item.code.toLowerCase().split('-')[0] === normalized.split('-')[0],
    );
    if (base) return base.code;
  } catch {
    // Intl may be unavailable; fall through to default
  }
